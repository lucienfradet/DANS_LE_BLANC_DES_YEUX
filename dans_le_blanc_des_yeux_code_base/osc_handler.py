...
2. Update the MotorController.__init__ method:
python
...
3. Update the _monitor_state method in MotorController:
python
...
4. Update the _start_movement and _motion_complete methods:
python
...
Continue reading pressure data while motors are moving
Add safety timeouts to prevent the system from getting stuck in a "moving" state
Add debug logging to track movement timing
Reduce the default motor timeout for quicker recovery
The most important change is the first one - modifying the OSC handler to read pressure data even when motors are moving, which directly addresses your issue.
