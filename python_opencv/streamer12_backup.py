...
def receive_camera_stream_interactive():
    """Receives and displays the camera streams interactively by pausing for user input."""
    global overlay_status, remote_overlay_status
    print("Waiting for camera frames... Press Enter to display each frame.")
    
    while True:
        # Receive data from the remote device camera
        ...
        # Display the frame
        cv2.namedWindow("Interactive Camera Stream", cv2.WINDOW_FULLSCREEN)
        cv2.setWindowProperty("Interactive Camera Stream", cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)

        resized_frame = cv2.resize(frame_front, (1024, 600))  # Resize the frame for display
        cv2.imshow("Interactive Camera Stream", resized_frame)

        # Allow user to quit the stream
...
def newEyeDetection():
    ...
    #cam = eyeCheckCam
    _, frame = cam.read()
    resized = cv2.resize(frame, (1920, 720)) #might want to rescale to 1920 x 1080 for our 1080p cameras
...
video_receive_thread = threading.Thread(target=receive_camera_stream_interactive, daemon=True)
